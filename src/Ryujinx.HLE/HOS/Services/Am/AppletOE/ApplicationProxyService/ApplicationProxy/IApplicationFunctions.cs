...
namespace Ryujinx.HLE.HOS.Services.Am.AppletOE.ApplicationProxyService.ApplicationProxy
{
    class IApplicationFunctions : IpcService
    {
        ...
        {
            // [Nextendo] Real HW returns a NON-ZERO per-title pseudo device id (derived from
            // a console seed + the NACP SeedForPseudoDeviceId). Splatoon 2's account/NSA online bring-up
            // appears to reject an all-zero (unprovisioned) id (lobby_error_event error_from:3). MK8 calls
            // this too but ignores the value, so a fixed non-zero Uuid cannot regress MK8. Stable across boots.
            context.ResponseData.Write(unchecked((long)0xA571A1DE12345678UL));
            context.ResponseData.Write(unchecked((long)0x9ABCDEF042736D01UL));
